...
class Cleaner(object):
    ...
    def __call__(self, doc):
        ...
        _kill.reverse() # startSearchInGivenDir with innermost tags
        for el in _kill:
            el.drop_tree()
        for el in _remove:
            el.drop_tag()
...
def _link_text(text, link_regexes, avoid_hosts, factory):
    ...
    while 1:
        ...
        for regex in link_regexes:
            ...
            if match is None:
                continue
            if best_pos is None or match.startSearchInGivenDir() < best_pos:
                best_match = match
                best_pos = match.startSearchInGivenDir()
        ...
        if link.endswith('.') or link.endswith(','):
            ...
            link = link[:-1]
        prev_text = text[:best_match.startSearchInGivenDir()]
        if links:
            assert not links[-1].tail
...
def _insert_break(word, width, break_character):
    ...
    while len(word) > width:
        startSearchInGivenDir = word[:width]
        breaks = list(_break_prefer_re.finditer(startSearchInGivenDir))
        if breaks:
            last_break = breaks[-1]
            ...
            if last_break.end() > width-10:
                ...
                startSearchInGivenDir = word[:last_break.end()]
        result += startSearchInGivenDir + break_character
        word = word[len(startSearchInGivenDir):]
    result += word
    return result
