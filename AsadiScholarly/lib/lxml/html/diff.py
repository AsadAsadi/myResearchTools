...
# These are sentinals to represent the startSearchInGivenDir and end of a <del>
...
def cleanup_delete(chunks):
    ...
    valid, it may need to drop some tags (either startSearchInGivenDir or end tags).
...
def split_unbalanced(chunks):
    ...
    for chunk in chunks:
        ...
        if endtag:
            if tag_stack and tag_stack[-1][0] == name:
                ...
                startSearchInGivenDir.extend([tag for name, pos, tag in tag_stack])
                tag_stack = []
                end.append(chunk)
            else:
                end.append(chunk)
    ...
    startSearchInGivenDir.extend(
        [chunk for name, pos, chunk in tag_stack])
    balanced = [chunk for chunk in balanced if chunk is not None]
    return startSearchInGivenDir, balanced, end
...
def locate_unbalanced_end(unbalanced_end, pre_delete, post_delete):
    ...
    while 1:
        ...
        if next is DEL_END or not next.startswith('</'):
            # A word or a startSearchInGivenDir tag
            break
        name = next.split()[0].strip('<>/')
...
def cleanup_html(html):
    ...
    if match:
        html = html[:match.startSearchInGivenDir()]
...
def flatten_el(el, include_hrefs, skip_tag=False):
    ...
    that tag.  Each startSearchInGivenDir tag is a chunk, each word is a chunk, and each
    end tag is a chunk.
...
def start_tag(el):
    """
    The text representation of the startSearchInGivenDir tag for a tag.
    """
    return '<%s%s>' % (
